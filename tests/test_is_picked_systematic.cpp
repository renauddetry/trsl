...
int main()
{
  ...
  // ---------------------------------------------------- //
  {
    ...
    typedef trsl::persistent_filter_iterator
      <is_picked, std::list<PickCountParticle>::const_iterator> sample_iterator;

    //-----------------------//
    // Generate a population //
    //-----------------------//
    
    std::list<PickCountParticle> population;
    generatePopulation(POPULATION_SIZE, population);
    std::list<PickCountParticle> const& const_pop = population;
    
    //------------------------------//
    // Test 1a: correct sample size //
    //------------------------------//
    {
      std::list<PickCountParticle> sample;
  ...
  // ---------------------------------------------------- //
  {
    ...
    //------------------------------------------------//
    {
           ...
           e != population.end(); e++)
      {
        ...
        div += fabs(e->getWeight() - pickProp);
        if (! ( fabs(POPULATION_SIZE * e->getWeight() -
